...
void readSensors() {
  if(myPort.available() > 0) {
    if (myPort.readBytesUntil(lf, inBuffer) > 0) {
      ...
      String [] inputStringArr = split(inputString, ' '); //was being split with a comma
...
void drawCube() {  
  pushMatrix();
    ...
    //translate like a quadrotor with axial rotation
    //the one in front of the sine is an arbitrary scaling factor (how fast it translates)
    translate(1 * sin(RwEst[0]), 1 * sin(RwEst[1]), 1 * sin(RwEst[2]));
