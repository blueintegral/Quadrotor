...
int RECpins[] = {8, 2, 4, 7};
int MotorPins[] = {3, 9, 10, 11};
int RECtime [4];
int ScaledRECin [4];

float RollPitchAngConst=10;
float RollPitchConst=.2;
...
void loop() {
  ...
  if (pulseIn(2, HIGH, 50000)){
    ...
    if(count > 0){
      ...
      for(int i=0; i<3; i++){
         Angles[i]=AccelAngles[i]/2+Angles[i]/2;
      }
    }
    else{
      ReadAccel(Accel);
    ...
    for (int i=0; i<20; i++){
        NewTime = millis();
        ReadAccel(Accel);
        NormalizeAccel(Accel, NormAccel);
        ReadGyro(Gyro);
        ...
        for(int i=0; i<3; i++){
          Angles[i]=Angles[i]+RotationalVelocity[i]*DT/1000;
        }
        
        for (int i=0; i<4; i++) {
          RECtime[i]=pulseIn(RECpins[i], HIGH, 1000000);
          ScaledRECin[i]=(RECtime[i]-900)/10;
          //Serial.print(ScaledRECin[i]);
          //Serial.print("  ");
      }
        Serial.println(" ");
        //RECin[0...3] = Throttle, Yaw, Pitch, Aileron
        //Motors Are Clockwise from First one positioned at -x direction.
        MotorOutput[0] = ScaledRECin[0] + (ScaledRECin[2]-50)*RollPitchConst + (ScaledRECin[1]-50)*YawConst + Angles[0]*RollPitchAngConst + RotationalVelocity[0]*RPDerivitiveConst + YawAngConst*Angles[3];
        MotorOutput[1] = ScaledRECin[0] + (ScaledRECin[3]-50)*RollPitchConst + (ScaledRECin[1]-50)*YawConst + Angles[1]*RollPitchAngConst + RotationalVelocity[1]*RPDerivitiveConst - YawAngConst*Angles[3];
        MotorOutput[2] = ScaledRECin[0] - (ScaledRECin[2]-50)*RollPitchConst - (ScaledRECin[1]-50)*YawConst + Angles[0]*RollPitchAngConst + RotationalVelocity[0]*RPDerivitiveConst + YawAngConst*Angles[3];
        MotorOutput[3] = ScaledRECin[0] - (ScaledRECin[3]-50)*RollPitchConst - (ScaledRECin[1]-50)*YawConst + Angles[1]*RollPitchAngConst + RotationalVelocity[1]*RPDerivitiveConst - YawAngConst*Angles[3];
        
        int ScalingMax = 100+50*RollPitchConst+50*YawConst+30*RollPitchAngConst+300*RPDerivitiveConst+20*YawAngConst;
        Serial.print(ScalingMax);
          Serial.print("  ");
        
        for (int i=0; i<4; i++) {
          MotorOutput[i]=MotorOutput[i]+127;
          if (MotorOutput[i]<0){
            MotorOutput[i]=0;
          }
